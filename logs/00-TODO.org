...
** Disjunction of PC in the antecedent
	 In some transition we have an antecedent of the form:

		(pc = 2 /\ (pc'=3 \/ pc'=7) /\ A) -> C

	 where C may include formulas of the form (pc'=42 -> foo). The problem is that 
	 simplify-pc tactic is capable extracting information from the antecedent
	 reasoning only about conjunctions of literals, and not through disjunctions.

	 The first idea was then to create a tactic that split the disjunction into
	 disjoint cases, converting a formula A->C into A1->C, A2->C and A3->C, where
	 A->C is valid iff A1->C, A2->C and A3->C are all valid. In this example, A1,
	 A2 and A3 are:

		A1 : (pc'=3) /\ (pc = 2 /\ (pc'=3 \/ pc'=7) /\ A)
		A2 : (pc'=7) /\ (pc = 2 /\ (pc'=3 \/ pc'=7) /\ A)
		A3 : (pc'!=3) /\ (pc'!=7) /\ (pc = 2 /\ (pc'=3 \/ pc'=7) /\ A)


	 The, we just need to propagate the pc equalities through the formulas.

	 I propose that only A1->C and A2->C should be checked for validity, as A3->C 
	 is trivially valid. Hence, I propose that we don't even generate A3->C. At
	 this moment, Leap does not generate A3->C as it only consumes unnecessary 
	 computation time.
...
** We need to improve the propositional-propagate and/or the simplify-pc tactic 
	 to let them reason when we have a fact that is a disjunction. For instance,
	 currently if we have:
		(pc=2 \/ pc=3 \/ pc=4) -> (pc=42 -> foo)
	 the tactic is not capable of reducing the VC to true.
...
** [2013-07-25]
*** Verification that skiplist of arbitrary height is valid


** [2013-07-30]
*** When normalizing a TSL formula, literals of the form (~ skiplist...) where 
		not normalized.
*** Modified representation of variable updates. So far we have been using
		Expression.ArrayUp(arr,t,v) to denote an update on a parametrized variable.
		However, when in to_fol mode, variables are not represented as arrays but a
		single variables. Hence, I need to do this modification, otherwise an update
		of a local variable as an array representation has no sense at all. Notice
		that this may bring problems with parametrized examples in the future. This
		modification is only for the time being for skiplist example, as it is a
		sequential one.


** [2013-07-31]
*** Fixed bug in PA/NC split function.
*** More support for skiplist example.


** [2013-08-01]
*** Fixed bug in path declaration on Z3 queries of TSLK.
*** Substitution of adr2set for path2set(getp(...)) in TSL to TSLK translation.
		This prevent the inclusion of null as element in the region.


** [2013-08-02]
*** Some VCs for skiplist solved
*** Added to arrangements the feature to denote that a certain element should be
		in the immediately following equivalence class of another element when
		constructing the arrangement. Now some VCs are much faster (for instance
		some VC verification time has been reduced from 9 min to 11 sec.)


** [2013-08-05]
*** Fix bug with arrangement generation.
*** Continue with support for skiplist


** [2013-08-06]
*** Updated Minisat Linux version to prevent End_of_file error on Devel.
...
** When translating ~skiplist, we convert it into a disjunction of conjunctions.  
	 One of such disjunction asserts that tail points at some level to an address
	 different than NULL. This is something of the form:

			0 = fresh_int1 /\

			maxLevel != fresh_int1 /\
			fresh_int1 <= fresh_int3 /\ fresh_int3 <= fresh_int2 /\

			fresh_cell = heap_prime[tail] /\
			fresh_cell = mkcell (fresh_elem, fresh_addrarr, fresh_int2) /\
			fresh_addr = fresh_addrarr[fresh_int3] /\
			fresh_addr != null

	 We should also assert that fresh_int2 is in fact bounded by maxLevel, so that
	 tail is pointing to a non null address on a valid level.
