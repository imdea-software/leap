...
struct
  ...
  and tidarrterm_to_str (tt:Expr.tidarr) : string =
    ...
    | Expr.TidArrayUp(tt,i,t) -> Printf.sprintf "(store %s %s %s)"
...
*)
  ...
  let z3_preamble (buf:B.t) (num_addr:int) (num_tid:int) (num_elem:int)
    ...
    B.add_string buf ";; THM Z3 Translation\n";
...
*)
  ...
  let formula_to_str (co:Smp.cutoff_strategy_t)
    ...
    clean_lists();
    set_configuration use_q;
    let _ = LeapDebug.debug "entering Z3ThmQuery.formula_to_str...\n" in
    let _ = GM.clear_sort_map sort_map in
    let _ = LeapDebug.debug "Z3ThmQuery will compute the cutoff...\n" in
...
(*      B.add_string buf extra_info_str ; *)
      ...
      B.add_string buf formula_str ;
      B.add_string buf ")\n(check-sat)" ;
      LeapDebug.debug "exiting Z3ThmQuery.formula_to_str...\n";
      B.contents   buf
