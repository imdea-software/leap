...
struct
  ...
  (*         (lambda (t::tid)                       *)
  (*             (and (s t) (r t)))))               *)
  let smt_intersectionth_def buf =
    B.add_string buf
  ...
  (*         (lambda (e::elem)                              *)
  (*             (and (s e) (r e)))))                       *)
  let smt_intersectionelem_def buf =
    B.add_string buf
  ...
  *)
  ...
  let smt_defs buf num_addr num_tid num_elem req_sorts req_ops =
    ...
    if List.mem Expr.SetTh req_sorts then
      begin
        ...
        smt_unionth_def buf ;
        smt_intersectionth_def buf ;
        smt_setdiffth_def buf ;
    ...
    if List.mem Expr.SetElem req_sorts then
      begin
        ...
        smt_unionelem_def buf ;
        smt_intersectionelem_def buf ;
        smt_setdiffelem_def buf ;
        smt_subseteqelem_def buf num_elem ;
        smt_eqsetelem_def buf num_elem
      end;
    (* Set2Elem *)
    if List.mem Expr.Set2Elem req_ops then smt_settoelems_def buf ;
    (* Firstlock *)
  ...
  and define_variables (buf:Buffer.t) (num_tids:int) (vars:Expr.VarSet.t) : unit =
    ...
    let varunk     = Expr.varset_of_sort vars Expr.Unknown  in
      ...
      Expr.VarSet.iter (smt_define_var buf vartid num_tids) vartid;
      Expr.VarSet.iter (smt_define_var buf vartid num_tids) varaddr;
      Expr.VarSet.iter (smt_define_var buf vartid num_tids) varcell;
      Expr.VarSet.iter (smt_define_var buf vartid num_tids) varsetth;
  ...
  let process_set (max_addrs:int) (s:Expr.set) : string =
    ...
    | Expr.AddrToSet (m,a) ->
        ...
        let a_str = addrterm_to_str a in
        let tmpbuf = B.create 1024 in
        B.add_string tmpbuf ("(assert (isaddr " ^a_str^ "))\n");
        for i = 0 to max_addrs do
          B.add_string tmpbuf
            ("(assert (isaddr (next" ^string_of_int i^ " " ^m_str^ " " ^a_str^ ")))\n")
        done;
        let auxbuf = ref ("(singleton " ^a_str^ ")") in
        for i = 1 to max_addrs do
