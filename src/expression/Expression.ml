...
and param_setint (pfun:variable option -> tid option) (s:setint) : setint =
  ...
  | EmptySetInt            -> EmptySetInt
  | SinglInt(i)            -> SinglInt(param_int pfun i)
...
and param_int (pfun:variable option -> tid option) (i:integer) : integer =
  match i with
    IntVal(i)           -> IntVal(i)
  | VarInt v            -> VarInt (set_var_th v (pfun (Some v)))
  | IntNeg(i)           -> IntNeg(param_int pfun i)
  | IntAdd(i1,i2)       -> IntAdd(param_int pfun i1,
                                  param_int pfun i2)
  | IntSub(i1,i2)       -> IntSub(param_int pfun i1,
                                  param_int pfun i2)
  | IntMul(i1,i2)       -> IntMul(param_int pfun i1,
                                  param_int pfun i2)
  | IntDiv(i1,i2)       -> IntDiv(param_int pfun i1,
                                  param_int pfun i2)
...
and param_atom (pfun:variable option -> tid option) (a:atom) : atom =
  ...
  | InInt(i,s)                 -> InInt(param_int pfun i,
                                        param_setint pfun s)
  ...
  | Less(i1,i2)                -> Less(param_int pfun i1,
                                       param_int pfun i2)
  | Greater(i1,i2)             -> Greater(param_int pfun i1,
                                          param_int pfun i2)
...
let rec norm_literal (gen_info:fresh_var_gen_t) (l:literal) : literal list =
  []


and norm_conjformula (gen_info:fresh_var_gen_t) (cf:conjunctive_formula)
      : conjunctive_formula =
  cf


and norm_formula (gen_info:fresh_var_gen_t) (phi:formula) : formula =
  phi


let normalize_literal (l:literal) : literal list =
  []

let normalize_conjformula (cf:conjunctive_formula) : conjunctive_formula =
  cf
