...
module Make (E:GenericExpression.S) =
  struct
    ...
    type vc_info = { 
      ...
      tid_constraint   : formula      ;
      
      rho             : formula ;   (* TRANSITION RELATION *)
    ...
    *)
    ...
    let vc_info_to_formula  (info:vc_info) (sup:support_t): formula =
    ...
    let dup_vc_info_with_goal (info:vc_info) (new_goal:formula) : vc_info =
      {
    ...
    and get_tid_constraint_from_info (info:vc_info) : formula = info.tid_constraint
    ...
    let rec get_antecedent (vc:verification_condition) : formula=
      vc.antecedent
    and get_consequent (vc:verification_condition) : formula=
      vc.consequent
    ...
    and get_tid_constraint (vc:verification_condition) : formula =
      get_tid_constraint_from_info vc.info
    and get_rho (vc:verification_condition) : formula =
      get_rho_from_info vc.info
    and get_goal (vc:verification_condition) : formula =
    ...
    let generic_simplifier (phi:formula) (simp_lit:E.literal-> polarity->formula) : formula =
      let is_true  (f:formula):bool = match f with F.True  -> true | _ -> false in
      let is_false (f:formula):bool = match f with F.False -> true | _ -> false in
      let rec simplify_f (f:formula) (pol:polarity): formula=
        match f with
    ...
    (*     | _                              -> false in *)
    (*   let simplify_pc (lit:E.literal) (pol:polarity) : formula = *)
    (*     match lit with *)
    ...
    (**************************************************************************)
    (* SUPPORT TACTICS, that generate support (formula list) from vc_info   *)
    (**************************************************************************)
    let generate_support (info:vc_info) : formula list =
      (* FIX THIS *)
      let (no_param,param) =
    ...
    let split_antecedent_pc (imp:implication) : implication list =
      let candidates (phi:formula) : formula list =
    ...
    *)
    ...
    let unify_support (vc:vc_info) : vc_info =
      let unify_tbl : (int, (E.tid list * formula) list) Hashtbl.t = Hashtbl.create 4 in
      List.iter (fun phi ->
