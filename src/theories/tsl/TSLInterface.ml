...
and addr_to_tsl_addr (a:E.addr) : SL.addr =
  ...
  | E.LastLocked _           -> raise(UnsupportedTslExpr(E.addr_to_str a))
...
and cell_to_tsl_cell (c:E.cell) : SL.cell =
  ...
  | E.MkSLCell (e,aa,tt,l) -> SL.MkCell (elem_to_tsl_elem e,
                                             ...
                                             int_to_tsl_int l)
  (* ALE: TSL receives two arguments, while current expression receives only one.
...
and atom_to_tsl_atom (a:E.atom) : SL.atom =
  ...
  | E.InTidPair _              -> raise(UnsupportedTslExpr(E.atom_to_str a))
  | E.InIntPair _              -> raise(UnsupportedTslExpr(E.atom_to_str a))
  ...
  | E.UniqueInt _              -> raise(UnsupportedTslExpr(E.atom_to_str a))
  | E.UniqueTid _              -> raise(UnsupportedTslExpr(E.atom_to_str a))
