...
latex_default_file = 'results.tex'
...
def error(msg):
	print_and_log ('[ ERROR		] ***  {}  ***'.format(msg))
...
def warning(msg):
	print_and_log ('[ WARNING ] ***  {}  ***'.format(msg))
...
def clean_examples(examples):
	print_and_log ('Cleaning examples...')
...
def lookup_results(results, name):
	try:
		...
		print_and_log ('! No results for ' + name)
		return []
...
def test_deployment(verbose):
	...
	if there_is_history:
		print_and_log ('History file found at "{}"'.format(history_path))
	...
	for k in [d for d in list_folders(testing_path) if d <> result_folder]:
		...
		middle_msg(k)
		for f in zips:
			print_and_log ('Unzipping example file {} ...'.format(f))
			unzip (k_path, f)
		...
		for ex in examples:
			...
			print_and_log ('Analyzing example {}:{}...'.format(k,ex))
			example_path = join(k_path,ex)
			scripts = list_files_of_type (example_path,'.sh')
			for sh in scripts:
				if k == 'safety':
					print_and_log ('For example {}, analyzing script {} ...'.format(ex, sh))
					...
					print_and_log ('Graph information located in ' + graph_file)
					...
					case_counter = 1
					for case in cases:
						print_info_and_log ('Analyzing case {} of {}: {}...'.format(case_counter, len(cases), case.name))
						case_counter += 1
					...
					print_and_log ('Liveness verification not implemented yet')
		

	summary_out.close()
	print_med_separator()
	print_and_log ('{} Changes detected'.format(len(changes)))
...
def deploy(examples):
	...
	for k in examples.keys():
		...
		for ex in examples[k]:
			...
			print_and_log ('Creating zip file for {}:{} ...'.format(k,ex))
	...
	print_and_log ('Examples will be deployed to "{}"'.format(deployment_path))
...
def generate_latex(filename):
	...
	values = read_summary_file(file_path)
	out = open (latex_default_file, 'w')
	out.seek(0)
	out.truncate()
	buf = '\\scriptsize\n'
	...
	for line in values:
		...
		buf = buf + ('${:>4}$ & {:<60} ' + ('& ${:>4}$' * 23) + ('& ${:>9}$' * 6) + '\\\\ \\hline\n').format(
							 ...
							 read(vc_calls, 'tslk[2]'),
							 read(vc_calls, 'tslk[3]'),
							 ...
							 info['R_TOTAL_DP'],
							 info['R_LEAP_TIME'],
							 info['R_TOTAL_ANALYSIS'])
		n = n + 1
	buf = buf + '\\end{tabular}'
	out.write (buf)
	out.close()
	print_and_log ('Latex copy exported to "{}"'.format(latex_default_file))
...
def main(argv):
	...
	if args.latex and (args.clean or args.test or args.deploy):
			print '-latex option is not compatible with -clean, -test or -deploy'
	...
	if args.latex:
		...
		if args.examples <> None:
			examples_in = open(join(current_path, args.examples.strip()))
			curr_cat = ''
